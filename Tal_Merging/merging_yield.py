...
unique_inner_df.to_csv('C:/Users/Inholland/Desktop/dataset/yield_unique_inner_join_test1.csv')
...
#fmt='.1f' - numbers inside the correlation
sns.heatmap(unique_inner_df.corr(), annot=True, linewidths=.5, fmt='.1f', ax=ax)
plt.show()
...
unique_inner_df1.to_csv('C:/Users/Inholland/Desktop/dataset/yield_unique_inner_join_no_duplicates_test2.csv')


len(unique_inner_df1.columns)
unique_inner_df1.columns

cols = list(unique_inner_df1.columns)

for col in cols:
    unique_inner_df1[cols.columns.add_suffix('_average')] = unique_inner_df1[[col, col]].mean(axis=1)

#taking the mean of _x and _y columns
unique_inner_df1["conv.factor_average"] = unique_inner_df1[["conv.factor_x", "conv.factor_y"]].mean(axis=1) #axis=1 <-row wise 0 = column wise
...
unique_inner_df1["conv.factor_x"].head() == unique_inner_df1["conv.factor_y"].head() #returns false
...
#unique_inner_df1.columns["conv.factor_average"] = unique_inner_df1.columns["conv.factor_x"].mean(axis=1) #axis=1 <-row wise 0 = column wise


unique_inner_df1["speed(km/h)"].value_counts() # mean: 6.230955  - count: 28206
unique_inner_df1["speedkmh"].value_counts() # mean: 6.228926  - count: 28206
unique_inner_df1["speed(km/h)"].describe()
unique_inner_df1["speedkmh"].describe()
#unique_inner_df1["_x"] = round(unique_inner_df["_x"], 3)
(unique_inner_df1["speed(km/h)"].head(100) == unique_inner_df1["speedkmh"].head(100))#.all() <-when I do all they = False but when I look at the first 5 columns they returen 4 - true 1-false
...
f, ax = plt.subplots(figsize=(18, 18))
#fmt='.1f' - numbers inside the correlation
sns.heatmap(unique_inner_df1.corr(), annot=True, linewidths=.5, fmt='.1f', ax=ax)
plt.show()
...
'''
#renaming attributes
"""unique_inner_df1 = unique_inner_df.rename(columns={'speedkmh': 'speed km/h', 'conv.factor_x': 'conv.factor', 'loadkg': 'load(kg)',
'X': 'lon(degr)', 'Y': 'lat(degr)', 'Hoogte': 'alt(m)', 'tarekg': 'tare(kg)', 'qual_x': 'qual'
'beltspdm': 'beltspd(m/s)', 'workwidth(': 'workwidth(m)',
